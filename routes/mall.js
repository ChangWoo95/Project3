...
router.get('/', function(req, res, next) {
   if(!req.session.islogined){
	...
	pool.getConnection(function(err, connection){
		...
		sql += "delete from onsale where TIMESTAMPDIFF(second,onsale.enddate,now()) > 0;";

		connection.query(sql, function(err, rows){
			if(err) console.error("로그인sql err",err);
			res.render('index',{rows: rows[0], session: req.session});
			connection.release();
		});
	});	    
});
...
router.get('/sale_manage', function(req, res, next) {
 ...
 router.get('/sale_detail/:I_id', function(req, res, next) {
	...
	pool.getConnection(function(err, connection){
		...
		connection.query(sql, num, function(err, rows){
			if(err) console.error("글 삭제 중 에러 발생 err : ", err);
			else res.render('sale_detail',{session: req.session, row: rows[0], now_d: date});	
			connection.release();
		});
	});
});
 ...
 router.post('/sale_detail/:I_id', function(req, res, next) {
	...
	pool.getConnection(function(err, connection){
		var num = req.params.I_id;
		var st_date = req.body.startdate;
		var ed_date = req.body.enddate;
		var sale = req.body.sale;

		var datas = [st_date,ed_date,sale,num];
		var sql = "insert into onsale(startdate,enddate,sale,I_id) values(?,?,?,?)";

		connection.query(sql, datas, function(err, rows){
			if(err) console.error("글 삭제 중 에러 발생 err : ", err);
			else res.send("<script>alert('할인상품등록이 완료되었습니다.');location.href='/mall/sale_manage';</script>");
...
router.get('/product_detail/:I_id', function(req, res, next) {
	...
	pool.getConnection(function(err, connection){
		...
		var sql = "select I.*,O.sale,O.startdate,O.enddate,TIMESTAMPDIFF(day,now(),O.enddate) as diff from item as I left join onsale as O on I.I_id = O.I_id and TIMESTAMPDIFF(second,O.startdate,now()) > 0 where I.I_id=? order by I.I_id;";
...
router.get('/cart', function(req, res, next) {
	pool.getConnection(function(err, connection){
		var pk = req.session.pk;
		var sql = "select item.*,cart.val,cart.CC_id,O.sale,O.startdate,O.enddate,TIMESTAMPDIFF(day,now(),O.enddate) as diff from cart,item left join onsale as O on item.I_id = O.I_id and TIMESTAMPDIFF(second,O.startdate,now()) > 0 where item.I_id = cart.I_id and cart.C_id=?;";
